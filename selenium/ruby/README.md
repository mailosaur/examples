...
## Reset a password using email - `password_reset_spec.rb`

Shows you how to perform an automated test for a password reset workflow:

```
rake spec SPEC=spec/password_reset_spec.rb
```
...
## Multi-Factor Authentication (MFA) via SMS - `otp_sms_spec.rb`
...
```
rake spec SPEC=spec/otp_sms_spec.rb
```
...
## Multi-Factor Authentication (MFA) via App - `otp_authenticator_spec.rb`
...
```
rake spec SPEC=spec/otp_authenticator_spec.rb
```
